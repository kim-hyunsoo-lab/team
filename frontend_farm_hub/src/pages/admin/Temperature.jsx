...
const Temperature = () => {
  ...
  const calculateStats = () => {
    ...
    // 0이 아닌 데이터만 필터링
    const validMaxData = temperatureData
      .filter((e) => e.maxTemp > 0)
      .map((e) => e.maxTemp);
    const validAvgData = temperatureData
      .filter((e) => e.avgTemp > 0)
      .map((e) => e.avgTemp);
    const validMinData = temperatureData
      .filter((e) => e.minTemp > 0)
      .map((e) => e.minTemp);
    ...
    <div className={styles.container}>
      <div className={styles.card}>
        <div className={styles.header}>
          <h1 className={styles.title}>축사 온도 데이터</h1>
          <Select
            value={dateRange}
            onChange={(e) => {
