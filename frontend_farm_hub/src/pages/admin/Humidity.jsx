...
const Humidity = () => {
  ...
  //마운트 시 데이터를 조회
  useEffect(() => {
    axios.get('/api/farms/humidity', {params : {each : dateRange}})
    .then(res => {
  ...
  const labels = humidityData.map((e) => {return(dayjs(e.createDate).format('YY-MM-DD'))})
  ...
  const data = {
    ...
    datasets: [
      {
        label: '최고습도',
        data: humidityData.map((e) => {return(e.maxHum)}),
      ...
      {
        label: '평균습도',
        data: humidityData.map((e) => {return(e.avgHum)}),
      ...
      {
        label: '최저습도',
        data: humidityData.map((e) => {return(e.minHum)}),
  ...
  const calculateStats = () => {
    ...
    // 0이 아닌 데이터만 필터링
    const validMaxData = humidityData.filter(e => e.maxHum > 0).map(e => e.maxHum);
    const validAvgData = humidityData.filter(e => e.avgHum > 0).map(e => e.avgHum);
    const validMinData = humidityData.filter(e => e.minHum > 0).map(e => e.minHum);

    // 선택한 기간 내 모든 maxHum 중 최댓값 (소수점 첫째자리)
    const maxValue = validMaxData.length > 0 ?
      Math.max(...validMaxData).toFixed(1) : '0.0';

    // 선택한 기간 내 모든 avgHum의 평균값 (소수점 첫째자리)
    const avgValue = validAvgData.length > 0 ?
      (validAvgData.reduce((acc, e) => acc + e, 0) / validAvgData.length).toFixed(1) : '0.0';

    // 선택한 기간 내 모든 minHum 중 최솟값 (소수점 첫째자리)
      ...
      <div className={styles.card}>
        ...
        <div className={styles.statsGrid}>
          <div className={`${styles.statCard} ${styles.statCardMax}`}>
            <p className={styles.statLabel}>최고 습도</p>
          ...
          <div className={`${styles.statCard} ${styles.statCardAvg}`}>
            <p className={styles.statLabel}>평균 습도</p>
          ...
          <div className={`${styles.statCard} ${styles.statCardMin}`}>
            <p className={styles.statLabel}>최저 습도</p>
